...
// @last_update                    07/05/2008 ||| XX     XX     XX X  XX  X  XX
// @version                              0.11 ||| XX     XXXXXX XX XXXXX  XXXXX
//
...
function check_submit_authorization($location) {
  ...
  if ($canIHaveAccess) {
    return true;   
      ...
      $user_ever_submitted_links_count = $db->get_var("SELECT count(link_id) FROM " . table_links . " WHERE link_author=".$current_user->user_id);
        
      
      // get actual average votes value   
      $user_vote_values = $db->get_results("SELECT vote_value FROM " .table_votes ."
     ...
     if ($user_vote_values) {                                                    
            foreach ($user_vote_values as $vote) {         
                    $sum_vv =$sum_vv +$vote->vote_value;  
            }             
