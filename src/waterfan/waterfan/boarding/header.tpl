...
<div id="login">{if $user_authenticated eq true}{#PLIGG_Visual_Welcome_Back#} <span style="text-transform:capitalize"><a href="{$URL_userNoVar}">{$user_logged_in}</a></span> | <a href="{$URL_logout}"> {#PLIGG_Visual_Logout#}</a>{/if}
			
		{if $user_authenticated neq true}<a href='{$URL_register}'>{#PLIGG_Visual_Register#}</a> | <a href='{$URL_login}'>{#PLIGG_Visual_Login_Title#}</a>{/if}</div>
		
<!-- START RSS -->
		<div class="rsslink"></div>
<!-- END RSS -->
...
<!-- START SEARCH -->
	<div class="search">
		...
		<form action="{$my_pligg_base}/search.php" method="get" name="thisform-search" id="thisform-search"
	        ...
	        <span class="rsslink">{if $URL_rss_page} <a href="{$URL_rss_page}" target="_blank"> RSS Feed &nbsp;<img src="{$my_pligg_base}/templates/{$the_template}/images/rss.png" align="top" border="0" alt="RSS" /> </a> {/if} </span>
	  </form>
	</div>
	<div class="clear"></div>
	<!-- END SEARCH -->
	...
	{assign var=sidebar_module value="categories"}{include file=$the_template_sidebar_modules."/wrapper.tpl"}{checkActionsTpl location="tpl_pligg_breadcrumb_end"}
