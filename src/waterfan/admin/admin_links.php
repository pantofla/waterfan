...
if($canIhaveAccess == 1) {
	...
	if(isset($_GET["mode"]) && sanitize($_GET["mode"], 3) == "search"){
		...
		$filtered = $db->get_results("SELECT * FROM " . table_links . " WHERE link_author LIKE '%".$pligg_keyword."%' OR link_title LIKE '%".$pligg_keyword."%' OR link_content LIKE '%".$pligg_keyword."%' ORDER BY link_date DESC LIMIT $offset,25");
		...
		$filtered = $db->get_results("SELECT * FROM " . table_links . " ORDER BY link_date DESC, link_author ASC LIMIT $offset,25");
	...
	if(isset($_GET["filter"])) {
		switch (sanitize($_GET["filter"], 3)) {
			case 'published':
				$filtered = $db->get_results("SELECT * FROM " . table_links . " where link_status = 'published' ORDER BY link_date DESC LIMIT $offset,25");
				$rows = $db->get_var("SELECT count(*) FROM " . table_links . " where link_status = 'published'");
				break;
		 	case 'upcoming':
				$filtered = $db->get_results("SELECT * FROM " . table_links . " where link_status = 'queued' ORDER BY link_date DESC LIMIT $offset,25");
				$rows = $db->get_var("SELECT count(*) FROM " . table_links . " where link_status = 'queued'");
				break;
		  case 'discard':
				$filtered = $db->get_results("SELECT * FROM " . table_links . " where link_status = 'discard' ORDER BY link_date DESC LIMIT $offset,25");
				$rows = $db->get_var("SELECT count(*) FROM " . table_links . " where link_status = 'discard'");
				break;
		  case 'all':
				$filtered = $db->get_results("SELECT * FROM " . table_links . " ORDER BY link_date DESC LIMIT $offset,25");
				$rows = $db->get_var("SELECT count(*) FROM " . table_links . "");
				break;
			case 'today':
			  $tsdt = date('Ymd000000', strtotime("now"));
				$fsdt = date('Ymd235959', strtotime("now"));
				$filtered = $db->get_results("SELECT * FROM " . table_links . " WHERE (link_date >= $tsdt AND link_date <= $fsdt) ORDER BY link_date DESC LIMIT $offset,25");
