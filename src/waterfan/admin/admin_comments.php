...
if($canIhaveAccess == 1) {
	...
	if(isset($_GET["mode"]) && sanitize($_GET["mode"], 3) == "search"){
		$filtered = $db->get_results("SELECT * FROM " . table_comments . " WHERE comment_content LIKE '%".sanitize($_GET["keyword"], 3)."%' ORDER BY comment_date DESC LIMIT $offset,25");
		$rows = $db->get_var("SELECT count(*) FROM " . table_comments . " WHERE comment_content LIKE '%".sanitize($_GET["keyword"], 3)."%'");		
	}
	else {
		$filtered = $db->get_results("SELECT * FROM " . table_comments . " ORDER BY comment_date DESC LIMIT $offset,25");
		...
		switch (sanitize($_GET["filter"], 3)) {
			...
			case $main_smarty->get_config_vars('PLIGG_Visual_Comments_Filter_Today'):
			  $tsdt = date('Ymd000000', strtotime("now"));
				...
				$filtered = $db->get_results("SELECT * FROM " . table_comments . " WHERE (comment_date >= $tsdt AND comment_date <= $fsdt) ORDER BY comment_date DESC LIMIT $offset,25");
