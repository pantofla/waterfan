...
<!-- START LEFT COLUMN -->
	...
	{/if}
		...
		{if $pagename eq "user"}<h1><span style="text-transform:capitalize">{$page_header}</span> <a href="{$user_rss, $view_href}" target="_blank"><img src="{$my_pligg_base}/templates/{$the_template}/images/rss.png" style="margin-left:6px;border:0;"></a></h1>{/if}
		</div>
		<!-- END BREADCRUMBS -->
