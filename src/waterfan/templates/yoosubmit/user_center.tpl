...
>
				<input type="hidden" name="view" value="search">
					...
					{/if}
				<input type="text" name="keyword" class="text-short" value="{$searchboxtext}" onfocus="if(this.value == '{$searchboxtext}') {ldelim}this.value = '';{rdelim}" onblur="if (this.value == '') {ldelim}this.value = '{$searchboxtext}';{rdelim}">
...
>
					<input type="hidden" name="view" value="search">
					<input type="text" name="keyword" class="text-short">
					<input type="submit" value="{#PLIGG_Visual_User_Search_Users#}" class="button">
					</form>
				</div><br/><br/>
			{/if}
	...
	{else}
		<br /><br /><h2>{#PLIGG_Visual_User_Profile_No_Friends#}</h2>
	{/if}</div>
{/if}
...
{if $user_view eq 'setting'}
	...
	<form action="{$my_pligg_base}/user_settings.php?login={$user_username}" method="post">
			...
			{foreach from=$category item=cat name="cate"}
				...
				<div style="width:145px;margin-right:10px;padding:5px;margin-bottom:10px;border:1px solid #E9DDAB;float:left;background-color:#FBF7E5;">
...
<div class="border-sections">
	...
	{else}
		<br /><br /><h2>{#PLIGG_Visual_User_Profile_No_Friends_2#}</h2>
	{/if}</div>
{/if}
...
{if $user_view eq 'profile'}{if $Allow_Friends neq "0"}	
			{if $user_authenticated eq true} 
<div class="border-sections">
		{if $Allow_Friends neq "0"}	
			{if $user_authenticated eq true} 
		...
		<div class="border-sections">
			...
			{if $friends}
				...
				{foreach from=$friends item=myfriend}
					...
					{/php}
					<tr>
					<td><img src="{$friend_avatar}" align="absmiddle"/></td>
