...
<h2>{#PLIGG_Visual_Submit2_Details#}</h2>
	<form action="{$URL_submit}" method="post" name="thisform" id="thisform" enctype="multipart/form-data" onSubmit="return checkForm()">
		...
		<input type="text" id="title" class="text-long" name="title" value="{if $submit_title}{$submit_title}{else}{$submit_url_title}{/if}" size="60" maxlength="120" />
	...
	<label>{#PLIGG_Visual_Submit2_Description#}: </label>{#PLIGG_Visual_Submit2_DescInstruct#}
		...
		{if $Submit_Show_URL_Input eq 1}
			...
			<input type="text" class="text-long" name="trackback" id="trackback"  value="{$submit_trackback}" size="60" />
			<br /><br/>
		{/if}
		</div>
...
<div class="border-sections">
		...
		{if $enable_tags}
			...
			<input type="text" id="tags" class="wickEnabled text-long" name="tags" value="{$tags_words}" size="60" maxlength="40" /><br /><br />
