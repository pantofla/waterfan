...
<form action="" method="post" id="thisform" enctype="multipart/form-data" >
	...
	<div class="border-sections"><h2>{#PLIGG_Visual_Submit2_Source#}</h2>
		<label>{#PLIGG_Visual_Submit2_NewsURL#}: </label>
		<a href="{$submit_url}">{$submit_url}</a><br/>
		{if $isAdmin eq 1}<input type="text" class="text-long" name="url" size=60 value ="{$submit_url}"><br />{/if}<br />
		<label for="url_title" accesskey="2">{#PLIGG_Visual_Submit2_URLTitle#}: </label>{$submit_url_title}
	</div>		

	<div class="border-sections"><h2>{#PLIGG_Visual_Submit2_Details#}</h2>
		<label>{#PLIGG_Visual_Submit2_Title#}: </label>
		<span>{#PLIGG_Visual_Submit2_TitleInstruct#}</span><br/>
		<input type="text" class="text-long" id="title" name="title" value="{$submit_title}" size="60" maxlength="120" />
		...
		{if $enable_tags}
			...
			<input type="text" class="text-long" id="tags" name="tags" value="{$tags_words}" size="60" maxlength="120" />
		{/if}
		...
		{if $canIhaveAccess eq 1}
			...
			&nbsp;&nbsp;<input type="radio" name="reason" value="other" onclick="notify.checked = 'true';">{#PLIGG_Visual_EditStory_Reason_other#} <input type="text" class="text" name="otherreason" size="50"><br/><br/>
		{/if}
