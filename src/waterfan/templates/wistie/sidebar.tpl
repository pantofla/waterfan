{if $pagename neq "submit"}
	...
	<!-- START LINKS -->
	            ...
	            <div class="links">
	            	<div class="sectiontitle">{if $Enable_Tags eq 'true'}<a href="{$URL_tagcloud}">{#PLIGG_Visual_Tags#}</a>{/if}</div>
	            </div>
	            <div class="links">
	            	<div class="sectiontitle">{if $Enable_Live eq 'false'} <a href='{$URL_live}'> {#PLIGG_Visual_Live#}</a>{/if}</div>
	            </div>
	<!-- END LINKS -->
