...
{checkActionsTpl location="tpl_pligg_story_start"}
	...
	{/if}
		...
		<div class="title" id="title-{$link_shakebox_index}">
			...
			<span class="subtext">
				...
				{if $url_short neq "http://" && $url_short neq "://"}
					(<a href="{$url}" {if $open_in_new_window eq true} target="_blank"{/if}  rel="nofollow">{$url_short}</a>)
				{else}
					({$No_URL_Name})
				{/if}
		...
		{checkActionsTpl location="tpl_pligg_story_body_start"}
			{if $use_thumbnails eq true && $pagename eq "story" && $url_short neq "http://" && $url_short neq "://"}
				<b><img src="http://images.websnapr.com/?url={$url_short}&size=T" style="float:left; padding-right:10px;padding-top:35px;"></b>
			{/if}		
	...
	<div class="storyfooter">
		...
		{if $user_logged_in neq $link_submitter}
			|&nbsp;
		...
		{if $enable_tags eq 'true'}
			{if $tags ne ''}
                ...
                | 
				{section name=thistag loop=$tag_array}
					<a href="{$my_base_url}{$my_pligg_base}/search.php?search={$tag_array[thistag]}">{$tag_array[thistag]}</a>
				{/section}
                {/if}
				</span> 
			{/if}
