...
/*
    ...
    - MODIFIED! BE CAREFUL WHEN UPDATING! */
(function (jQuery) {
...
jQuery(document).ready(function () {
  ...
  jQuery("a.reply").click(function () {
    ...
    jQuery('#reply-' + target).slideDown(333);
    jQuery("#recaptcha_widget_div").appendTo('#reply-' + target+' .captcha-box');
  });
  jQuery("a.cancel-reply").click(function () {
    comment_id = jQuery(this).attr('href');
    target = comment_id.slice(comment_id.search('-') + 1, comment_id.length);
    jQuery('#reply-' + target).slideUp(333);
    jQuery("#recaptcha_widget_div").appendTo('form#submit-comment .captcha-box-main');
  });
  /*
  if(!wpCaptions){
